...
#include "iRobotCreateService_broadcaster.h"
...
void Create_impl::set_packets(RR_SHARED_PTR<RobotRaconteur::Wire<RR_SHARED_PTR<SensorPacket > > > value)
{
	...
	m_packetsBroadcaster = boost::make_shared<RobotRaconteur::WireBroadcaster<RR_SHARED_PTR<SensorPacket > > >();
	m_packetsBroadcaster->Init(value);
}
...
void Create_impl::SendSensorPacket(uint8_t id, boost::shared_ptr<RRArray<uint8_t> > data)
{
	...
	m_packetsBroadcaster->SetOutValue(p);
}
